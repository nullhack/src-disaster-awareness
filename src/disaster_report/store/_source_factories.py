...
def build_usgs(
    ...
    session.add(
        FactUsgsEarthquake(
            ...
            type_key=store._type_key(session, raw.disaster_type),
...
def build_who(
    ...
    session.add(
        FactWhoDon(
            ...
            source_key=store._source_key(session, raw.source_name),
            country_key=store._country_key(session, raw.country),
            publication_date_key=store._date_key(session, parse_date(raw.report_date)),
            don_id=natural,
...
def build_healthmap(
    ...
    session.add(
        FactHealthmapAlert(
            ...
            source_key=store._source_key(session, raw.source_name),
            country_key=store._country_key(session, raw.country),
            alert_date_key=store._date_key(session, parse_date(raw.report_date)),
            alert_id=natural,
